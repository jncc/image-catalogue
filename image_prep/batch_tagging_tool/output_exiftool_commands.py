...
# Title: Exiftool command writer

# Authors: Matear, L., Duncan, G. (2018)                                                  Email:
# Version Control: 1.0

# Script description:
# Uses batch tagging method but does not perform exiftool commands, instead generating the commandlines required for
# later manual tagging
...
if __name__ == '__main__':
    ...
    with open('{}_exiftool_log.csv'.format(time.strftime('%Y%m%d%H%M')), 'w', newline='') as exiftoolLog:
        logWriter = csv.writer(exiftoolLog, delimiter=",")
        logWriter.writerow(["Path", "Command"])
        allArgs = []
        for index, row in imageProforma.iterrows():
            ...
            try:
                ...
                strArgs = '" "'.join(fileExifToolArgs)
                logWriter.writerow([filePath, strArgs])
